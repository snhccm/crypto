...
class codec{
  ...
  public function encrypt(){
    ...
    $block['x'] = floor($image['width'] / 8);
    $block['y'] = floor($image['height'] / 8);

    // 根据随机函数得到随机序列
    $random = new random($this->seed);

    $collect = $random->sequence($image['width'] * $image['height']);
    // print_r($list);
    ...
    for ($y = 0; $y < $image['height']; $y++) {
      for ($x = 0; $x < $image['width']; $x++) {
        ...
        // 计算目的地图像的坐标
        $nx = $collect[$index] % $block['x'];
        $ny = floor($collect[$index] / $block['y']);

        // 执行回调函数
        $dist[] = $this->block($x,$y,$image,$nx,$ny);
  ...
  public function block($x, $y, $image, $nx, $ny){
    $dststart = ($y * $image['width'] + $x) * 8;
    $srcstart = ($nx * $image['width'] + $ny) * 8;

    $dist = array();
    //根据8像素分割
    for($a = 0; $a < 8; $a++){
      for($b = 0; $b < 8; $b++){
        $dist[$dststart +$b] = $image['data'][$dststart + $b];
      }
      $dststart += $image['width'];
      $srcstart += $image['width'];
    }
    return $dist;
  }
